namespace KnowTest.Web.Helpers;
...
class ModuleHelper
{
    internal static void InitAppModules()
    {
        Config.OnAddModule = modules =>
        {
            ...
            var bizApply = GetModule("BizApply", "业务申请", "appstore", ModuleType.Menu, 2);
    ...
    private static SysModule GetModule(string code, string name, string icon, ModuleType type, int sort)
    {
        return new SysModule { ParentId = "0", Code = code, Name = name, Icon = icon, Target = type.ToString(), Sort = sort, Enabled = true };
    }
